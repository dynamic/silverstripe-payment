...
class PaymentProcessor extends Controller {
	...
	/**
	 * Actions are not allowed on this controller.
	 */
	...
	/**
	 * Updates the model with gateway result information
	 *
	 * @param PaymentGateway_Result $result
	 */
	public function finalise($result) {
		 
		/*
		 * //Should be part of Gateway result handling
	...
	public function doRedirect() {
		...
		//Session::set('PaymentID', $this->payment->ID); //ID should already be known by app, and may be indirectly referenced by redirectURL eg: store/order/23 (has payment relationnship)
...
class PaymentProcessor_MerchantHosted extends PaymentProcessor {
	...
	public function getFormRequirements() {
		...
		return $required->appendRequiredFields(
			new RequiredFields('FirstName', 'LastName', 'CardNumber', 'DateExpiry', 'Cvc2')
		);
	}
}
...
class PaymentProcessor_GatewayHosted extends PaymentProcessor {
	...
	/*
	 * Attempt to set up references to gateway, and model
	 */
	function init(){
		parent::init();
		$this->payment = $this->getPaymentObject();
		if(!$this->payment){
			$this->doRedirect(); //no payment means we need to end processing
			return false;
		}
	}
	...
	protected function getPaymentObject() {
		//TODO: provide security from affecting payments not belonging to current user
		return DataObject::get_by_id('Payment', (int)$this->request->param('OtherID'));
	}
	
	/*
	 * Preparation to send user to external website for payment handling.
	*/
	public function capture($data) {
		parent::capture($data);
		// Set the return links
		// TODO: Allow custom return url
		$returnURL = Director::absoluteURL(Controller::join_links(
				$this->link(),'gatewayreturnpoint',$this->methodName,$this->payment->ID
			));
		$cancelURL = Director::absoluteURL(Controller::join_links(
				$this->link(),'cancel',$this->methodName,$this->payment->ID
			));
	...
	/**
	 * This action is called by an external gateway.
	 */
	public function gatewayreturnpoint($response) {
	...
	public function cancel($response) {
		return $this->finalise(new PaymentGateway_Result(PaymentGateway_Result::INCOMPLETE));
	}
